...
export default function SignUpBox2() {
  ...
  const addUser = async (email, uniID) => {
    try {
      ...
      const response = await axios.post('http://classmateas.azurewebsites.net/adduser', { email, uniID });
  ...
  const fetchUserID = async (email) => {
    try {
      const response = await axios.get(`http://classmateas.azurewebsites.net/email/${email}/userID`);
